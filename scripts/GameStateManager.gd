...
func reset_all_game_systems():
	...
	# 7. Emit game restart signal for any other systems
	var game_manager = get_node_or_null("/root/GameManager")
	if game_manager:
		game_manager.game_restart_requested.emit()
	
	# 8. Reset GameManager state
	if game_manager:
